...
tf.app.flags.DEFINE_integer("max_input_sequence_len", 3, "Maximum input sequence length.")
tf.app.flags.DEFINE_integer("max_output_sequence_len", 6, "Maximum output sequence length.")
tf.app.flags.DEFINE_integer("rnn_size", 256, "RNN unit size.")
tf.app.flags.DEFINE_integer("attention_size", 256, "Attention size.")
tf.app.flags.DEFINE_integer("num_layers", 6, "Number of layers.")
...
tf.app.flags.DEFINE_string("log_dir", "./logcool", "Log directory")
tf.app.flags.DEFINE_string("data_path", "./data/train.csv", "Data path.")
...
class ConvexHull(object):
  ...
  def read_data(self):
    ...
    inp=[]
    outp=[]
    recs=pd.read_csv(FLAGS.data_path,names=['T','secH','secC','Qm','obj'])
    nprecs=recs.values
    
    for i in range(nprecs.shape[0]):
        inp.append(ast.literal_eval(nprecs[i][0]))
        outp.append(ast.literal_eval(nprecs[i][2]))
    #print('inp=',inp)
    print('outp=',outp)
    ...
    for j in range(inp.__len__()):
        ...
        enc_input = np.array(enc_input).reshape([-1,2],order='F')
        enc_input=np.fliplr(enc_input)
